...
class BabelifyStream extends stream.Transform {
    constructor(opts) {
        super();
        this._data = [];
        this._babelCache = opts.babelCache;
    ...
    _flush(callback) {
        ...
        var hash;
        if (this._babelCache) {
            hash = createHash('sha1').update(data, 'utf8').digest('hex');
            try {
                cachedCode = fs.readFileSync(path.join(this._babelCache, hash + '.js'), 'utf8');
            } catch (ex) {
                if (ex.code !== 'ENOENT') {
                    throw ex;
                }
        ...
        const addCodeAndCallback = (code) => {
            this.push(code);
            callback();
        };

        if (cachedCode !== undefined) {
            addCodeAndCallback(cachedCode);
        } else {
